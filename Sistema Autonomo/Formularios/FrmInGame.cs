        ...
        private void AtualizaPiratasNoMapa()
        {
            ...
            foreach (var historicoItem in diferencaHistorico)
            {
                if (!partida.HistoricoPartida.Contains(historicoItem))
                {
                    ...
                    if (dados[2] != "@")
                    {
                        partida.Tabuleiro.CasasDoTabuleiro.ForEach(tabuleiro =>
                        {
                            if (tabuleiro.NumeroCasa == int.Parse(dados[3]))
                            {
                                tabuleiro.PiratasDaCasa.ForEach(pirata =>
                                {
                            ...
                            else if (tabuleiro.NumeroCasa == int.Parse(dados[4]))
                            {
                                foreach (var pirata in tabuleiro.PiratasDaCasa)
                                {
